...
def adminlist(update, context):
    ...
    for admin in administrators:
        ...
        if status == "creator":
            ...
            if custom_title:
                text += f"<code> 鈹椻攣 {html.escape(custom_title)}</code>\n"
    ...
    for admin in normal_admin_list:
        text += "\n<code> 鈥� </code>{}".format(admin)

    for admin_group in custom_admin_list.copy():
        if len(custom_admin_list[admin_group]) == 1:
            text += "\n<code> 鈥� </code>{} | <code>{}</code>".format(
    ...
    for admin_group, value in custom_admin_list.items():
        text += "\n馃毃 <code>{}</code>".format(admin_group)
        for admin in value:
            text += "\n<code> 鈥� </code>{}".format(admin)
        text += "\n"
...
__mod_name__ = "Admin"
__command_list__ = [
    "adminlist", "admins", "invitelink", "promote", "fullpromote", "lowpromote", "midpromote", "demote", "admincache"
]
__handlers__ = [
    ADMINLIST_HANDLER, PIN_HANDLER, UNPIN_HANDLER, INVITE_HANDLER,
    PROMOTE_HANDLER, FULLPROMOTE_HANDLER, LOW_PROMOTE_HANDLER, MID_PROMOTE_HANDLER, DEMOTE_HANDLER, SET_TITLE_HANDLER, ADMIN_REFRESH_HANDLER
]
